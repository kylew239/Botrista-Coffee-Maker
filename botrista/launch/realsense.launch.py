...
def generate_launch_description():
    return LaunchDescription(
        [
            ...
            GroupAction(
                actions=[
                    SetRemap(src="/camera/d435i/color/image_raw", dst="/image_raw"),
                    SetRemap(src="/camera/d435i/color/camera_info", dst="/camera_info"),
                ...
                condition=IfCondition(
                    EqualsSubstitution(
                        left=LaunchConfiguration("hardware_type"), right="real"
                    )
                ),
            ),
            IncludeLaunchDescription(
            ...
            Node(
                ...
                ros_arguments=[
                    "--params-file",
                    PathJoinSubstitution(
                        [FindPackageShare("botrista"), "config", "tag.yaml"]
                    ),
                ],
            ),
            Node(
