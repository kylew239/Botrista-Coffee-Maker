"""
Node which include actions to pick up, place and pour pot
...
class Pot(Node):
    def __init__(self):
        ...
        self.pick_pot_client = ActionServer(
            self,
            EmptyAction,
            "pick_pot",
            self.pick_pot_cb,
        ...
        self.place_pot_client = ActionServer(
            self,
            EmptyAction,
            "place_pot",
            self.place_pot_cb,
        ...
        self.pour_pot_server = ActionServer(
            self,
            EmptyAction,
            "pour_pot",
            self.pour_pot_cb,
        ...
        self.grasp_pose = Pose(
            position=Point(x=0.02, y=0.0, z=0.0), orientation=Quaternion()
        )
        ...
        self.top_tf = TransformStamped(
            header=Header(
                frame_id="filtered_pour_over_tag", stamp=self.get_clock().now().to_msg()
            ),
    ...
    async def place_pot_cb(self, goal_handle):
        ...
        """
        approach_pose = Pose(
            position=Point(x=0.0, y=0.0, z=-0.1), orientation=Quaternion()
        )

        grasp_pose = Pose(
            position=Point(x=0.02, y=0.0, z=-0.02), orientation=Quaternion()
        )
    ...
    async def pour_pot_cb(self, goal_handle):
        """Pour coffee to cup from the pot
        ...
        """
        try:
            cup_tf = self.buffer.lookup_transform("panda_link0", "cup_location", Time())
        except Exception:
            self.get_logger().error("No transform found")
            return
        ...
        spout_tf = TransformStamped(
            header=Header(
                frame_id="panda_link0", stamp=self.get_clock().now().to_msg()
            ),
            transform=Transform(
                translation=Vector3(x=0.0, y=-0.25, z=0.02), rotation=Quaternion()
            ),
        )
        ...
        cup_tf_pour_pose = Pose(
            ...
            orientation=Quaternion(x=-0.5, y=-0.5, z=0.5, w=-0.50),
        )

        cup_tf_pour_pose_2 = Pose(
            position=Point(x=0.0, y=0.31, z=0.25),
            orientation=Quaternion(x=0.4055798, y=0.4055798, z=-0.579228, w=0.579228),
        )
        ...
        retreat_pose = tf2_geometry_msgs.do_transform_pose(retreat_pose, spout_tf)
        retreat_pose = tf2_geometry_msgs.do_transform_pose(retreat_pose, cup_tf)
        ...
        # Transform to cup
        approach_pose = tf2_geometry_msgs.do_transform_pose(spout_approach_pose, cup_tf)
        pour_pose = tf2_geometry_msgs.do_transform_pose(spout_pour_pose, cup_tf)
        pour_pose_2 = tf2_geometry_msgs.do_transform_pose(spout_pour_pose_2, cup_tf)
